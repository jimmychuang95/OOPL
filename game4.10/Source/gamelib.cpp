/*
 * gamelib.cpp: セ郎纗や穿笴栏闽classimplementation
...
{
	//
	// digit[i]class varibale┮ゲ斗磷狡LoadBitmap
	//
	if (!isBmpLoaded) {
...
void CInteger::SetTopLeft(int nx, int ny)		// 盢笆礶オà畒夹簿 (x,y)
{
	x = nx; y = ny;
}
...
{
	GAME_ASSERT(isBmpLoaded, "CInteger: 叫磅︽LoadBitmap礛ShowBitmap");
	int nx;		// 陪ボ计 x 畒夹
	int MSB;	// 程オ娩(才腹)计计
	if (n >= 0) {
		MSB = n;
...
void CGameState::ShowInitProgress(int percent)
{
	...
	CDDraw::BltBackColor(DEFAULT_BG_COLOR);		// 盢 Back Plain 额箇砞肅︹
	CMovingBitmap loading;						// 禟loading瓜ボ
	...
	CPen *pp, p(PS_NULL, 0, RGB(0,0,0));		// 睲埃pen
	pp = pDC->SelectObject(&p);

	CBrush *pb, b(RGB(0,255,0));				// 礶厚︹ progress
	pb = pDC->SelectObject(&b);
	pDC->Rectangle(x1,y1,x2,y2);				

	CBrush b1(DEFAULT_BG_COLOR);				// 礶堵︹ progrssいみ
	pDC->SelectObject(&b1);
	pDC->Rectangle(progress_x1,progress_y1,progress_x2_end,progress_y2);

	CBrush b2(RGB(255,255,0));					// 礶独︹ progrss秈
	pDC->SelectObject(&b2);
	pDC->Rectangle(progress_x1,progress_y1,progress_x2,progress_y2);

	pDC->SelectObject(pp);						// 睦 pen
	pDC->SelectObject(pb);						// 睦 brush
	CDDraw::ReleaseBackCDC();					// 奔 Back Plain  CDC
	//
	// 狦琌よノCDC杠ぃ璶й硂︽玥棵辊穦皗脅
	//
	CDDraw::BltBackToPrimary();					// 盢 Back Plain 禟棵辊
}
...
/////////////////////////////////////////////////////////////////////////////
// CGame: Game Class
// 硂class琌笴栏facade琌MFC籔笴栏篈爵假狦ぃ糤┪搭ぶ
// 笴栏篈杠ぃノ恨硂classざ籔龟
/////////////////////////////////////////////////////////////////////////////
...
: NUM_GAME_STATES(4)
{
	...
	//gameStateTable[GAME_STAGE_TWO] = new CGameStageTwo(this);
...
void CGame::OnDraw()
{
	CDDraw::BltBackColor(DEFAULT_BG_COLOR);	// 盢 Back Plain 额堵
	gameState->OnDraw();					// 陪ボ笴栏い–じ
	if (!running) {
		//
		// 狦既氨篈玥陪ボCtrl-Q...
		//
...
bool CGame::OnIdle()  // эぃ璶эOnIdle()τ莱эOnMove()のOnShow()
{
	if (suspended) {
		running = false;
		suspended = false;
	...
	//
	// 祘Α北笴栏秈︽硉猔種ㄆ兜
	// 1. ノDebug mode浪跌–Ω癹伴奔丁丁t
	// 2. 眖Ω瞒秨OnIdle()丁﹚33msぃ埃ㄤ丁ぃt
	//
	...
	CSpecialEffect::SetCurrentTime();	// 砞﹚瞒秨OnIdle()丁
	return true;
}

void CGame::OnInit()	// OnInit() 祘Α秨﹍磅︽Ω
{
	//
	// 币笆睹计
	//
	...
	// 秨币DirectX酶瓜ざ
	//
	CDDraw::Init(SIZE_X, SIZE_Y);							// 砞﹚笴栏秆猂
	//
	// 秨币DirectXざ
	//
...
{
	if (running)
		if ((nFlags & 0x4000) == 0) // 埃auto repeat
